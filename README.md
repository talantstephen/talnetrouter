...
[🚀 Quick Start](#-quick-start) • [💡 Features](#-key-features) • [📖 Setup](#-setup-guide) • [🌐 GitHub](https://github.com/talantstephen/talnetrouter)
...
TalNet Router works seamlessly with all major AI coding tools:
...
Set `X-TalNetRouter-Token-Saver: off` (legacy `X-TalNet Router-Token-Saver` still accepted) to bypass all token savers for one chat request.
...
Headroom is optional and runs separately. TalNet Router calls Headroom's local `/v1/compress` endpoint, then keeps normal routing, fallback, auth, and usage tracking:

```
Client → TalNet Router → Headroom /v1/compress → TalNet Router → provider
```
...
If Headroom is down or returns an error, TalNet Router fails open and sends the original request.
...
- Your CLI tool sends OpenAI format → TalNet Router translates → Provider receives native format
...
> TalNet Router itself **never charges** you anything. You only pay providers directly (if using paid services).
>
...
### 📊 Understanding TalNet Router Costs & Billing

**TalNet Router Billing Reality:**

✅ **TalNet Router software = FREE forever** (open source, never charges)  
...
❌ **TalNet Router never sends invoices** or charges your card
...
- **Cheap providers** (GLM, MiniMax): Pay them directly, TalNet Router just routes
- **FREE providers** (iFlow, Kiro, Qwen): Genuinely free forever, no hidden charges
- **TalNet Router**: Never charges anything, ever
...
The dashboard tracks your token usage and displays **estimated costs** as if you were using paid APIs directly. This is **not actual billing** - it's a reference to show how much you're saving by using free models or existing subscriptions through TalNet Router.
...
<details>
<summary><b>💳 Will I be charged by TalNet Router?</b></summary>

**No.** TalNet Router is free, open-source software that runs on your own computer. It never charges you anything.

**You only pay:**

- ✅ **Subscription providers** (Claude Code $20/mo, Codex $20-200/mo) → Pay them directly on their websites
- ✅ **Cheap providers** (GLM, MiniMax) → Pay them directly, TalNet Router just routes your requests
- ❌ **TalNet Router itself** → **Never charges anything, ever**

TalNet Router is a local proxy/router. It doesn't have your credit card, can't send invoices, and has no billing system. It's completely free software.
...
TalNet Router just routes your requests to them - there's no "catch" or future billing from TalNet Router itself. They're truly free services, and TalNet Router makes them easy to use with fallback support.
...
3. **Use subscription providers last:**
   ...
   - TalNet Router helps maximize their value through quota tracking
...
TalNet Router's smart fallback prevents surprise charges:

**Scenario:** You're on a coding sprint and blow through your quotas

**Without TalNet Router:**
...
**You're in control:** Set spending limits per provider in dashboard, and TalNet Router respects them.
...
**Pro Tip:** Use Opus for complex tasks, Sonnet for speed. TalNet Router tracks quota per model!
...
> **Note:** OpenClaw only works with local TalNet Router. Use `127.0.0.1` instead of `localhost` to avoid IPv6 resolution issues.
...
# Or use PM2
npm install -g pm2
pm2 start npm --name talnetrouter -- start
pm2 save
pm2 startup
```
...
- Docker Hub: [`talantstephen/talnetrouter`](https://hub.docker.com/r/talantstephen/talnetrouter)
- GHCR: [`ghcr.io/talantstephen/talnetrouter`](https://github.com/talantstephen/talnetrouter/pkgs/container/talnetrouter)
...
```bash
docker logs -f talnetrouter
docker restart talnetrouter
docker stop talnetrouter && docker rm talnetrouter
docker pull talantstephen/talnetrouter:latest   # update to latest
```

**Data persistence:** `$HOME/.talnetrouter/db/data.sqlite` on host ↔ `/app/data/db/data.sqlite` in container. Legacy `$HOME/.9router` is auto-migrated on first start.
...
- Both `${DATA_DIR}` and `~/.talnetrouter` resolve to the same location in a Docker container — the symlink `/root/.talnetrouter -> /app/data` (plus legacy `/root/.9router`) is created at build time.
...
> **Migrating from TalNet Router?** Your data at `~/.9router` is auto-copied to `~/.talnetrouter` on first start. The `9router` CLI bin still works as an alias for `talnetrouter`.
...
- Auto-refreshed by TalNet Router
...
- **Website**: [github.com/talantstephen/talnetrouter](https://github.com/talantstephen/talnetrouter)
- **GitHub**: [github.com/talantstephen/talnetrouter](https://github.com/talantstephen/talnetrouter)
- **Issues**: [github.com/talantstephen/talnetrouter/issues](https://github.com/talantstephen/talnetrouter/issues)

---

## 👥 Contributors

Thanks to all contributors who helped make TalNet Router better!

[![Contributors](https://contrib.rocks/image?repo=talantstephen/talnetrouter&max=150&columns=15&anon=1&v=20260309)](https://github.com/talantstephen/talnetrouter/graphs/contributors)

---

## 📊 Star Chart

[![Star Chart](https://starchart.cc/talantstephen/talnetrouter.svg?variant=adaptive)](https://starchart.cc/talantstephen/talnetrouter)

## 🔀 Forks

**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — A full-featured TypeScript fork of TalNet Router. Adds 36+ providers, 4-tier auto-fallback, multi-modal APIs (images, embeddings, audio, TTS), circuit breaker, semantic cache, LLM evaluations, and a polished dashboard. 368+ unit tests. Available via npm and Docker.
...
- **[RTK](https://github.com/rtk-ai/rtk)** ![Stars](https://img.shields.io/github/stars/rtk-ai/rtk?style=flat&color=yellow) — Rust token-saver. TalNet Router ports its compression pipeline to JS → **−20-40% input tokens** on every request.
- **[Caveman](https://github.com/JuliusBrussee/caveman)** ![Stars](https://img.shields.io/github/stars/JuliusBrussee/caveman?style=flat&color=yellow) by **[@JuliusBrussee](https://github.com/JuliusBrussee)** — viral _"why use many token when few token do trick"_. TalNet Router adapts its prompt → **−65% output tokens**.
- **[Ponytail](https://github.com/DietrichGebert/ponytail)** ![Stars](https://img.shields.io/github/stars/DietrichGebert/ponytail?style=flat&color=yellow) by **[@DietrichGebert](https://github.com/DietrichGebert)** — _"lazy senior dev"_ skill. TalNet Router injects its YAGNI-first ladder → **fewer tokens, less code, shorter diffs**.

Huge thanks to these authors — without their work, TalNet Router's token-saving features wouldn't exist. ⭐ them on GitHub!
