...
> 🎬 **Made a video about 9Router?** Submit a [Pull Request](https://github.com/talantstephen/talnetrouter/pulls) adding your video to this section — we'll merge it!
...
Set `X-TalNetRouter-Token-Saver: off` (legacy `X-9Router-Token-Saver` still accepted) to bypass all token savers for one chat request.
...
> **Migrating from 9Router?** Your data at `~/.9router` is auto-copied to `~/.talnetrouter` on first start. The `9router` CLI bin still works as an alias for `talnetrouter`.
