/**
 * `talnetrouter xai video` (alias `9router xai video`) — generate a Grok Imagine video through the local
 * TalNet Router gateway and save the result as an MP4 file.
 *
...
const HELP = `
Usage: talnetrouter xai video --prompt "..." [options]

Generate a Grok Imagine video via your local TalNet Router gateway
...
Options:
  ...
  --api-key <key>         TalNet Router API key (or env NINE_ROUTER_API_KEY)
...
async function run(argv) {
  ...
  try {
    ...
    const connectionId = create.headers["x-talnetrouter-connection-id"] || create.headers["x-9router-connection-id"] || null;
