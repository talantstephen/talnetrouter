...
function buildMenuItems(port, autostartEnabled) {
  return [
    { title: `TalNet Router (Port ${port})`, tooltip: "Server is running", enabled: false },
...
function initWindowsTray(options) {
  ...
  try {
    ...
    trayInstance = initWinTray({
      iconPath,
      tooltip: `TalNet Router - Port ${port}`,
      items,
      onClick: (index) => {
  ...
  try {
    ...
    if (isV2) {
      ...
      trayInstance.ready().catch((err) => {
        process.stderr.write(`[talnetrouter] tray failed to start: ${err && err.message ? err.message : err}\n`);
      });
    } else {
    ...
    process.stderr.write(`[talnetrouter] tray init error: ${err.message}\n`);
    return null;
  }
}
