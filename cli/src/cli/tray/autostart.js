...
const APP_LABEL = "com.talnetrouter.autostart";
const LEGACY_APP_NAME = "9router";
const LEGACY_LABEL = "com.9router.autostart";
 ...
 *   2. `process.argv[1]` if it's our cli.js — true when talnetrouter is currently
...
function isAutoStartEnabled() {
  ...
  try {
    if (platform === "darwin") {
      for (const label of [APP_LABEL, LEGACY_LABEL]) {
      ...
      const legacyStartup = path.join(process.env.APPDATA || "", "Microsoft", "Windows", "Start Menu", "Programs", "Startup", `${LEGACY_APP_NAME}.vbs`);
      return fs.existsSync(startupPath) || fs.existsSync(legacyStartup);
    } else if (platform === "linux") {
      const desktopPath = path.join(os.homedir(), ".config", "autostart", `${APP_NAME}.desktop`);
      const legacyDesktop = path.join(os.homedir(), ".config", "autostart", `${LEGACY_APP_NAME}.desktop`);
      return fs.existsSync(desktopPath) || fs.existsSync(legacyDesktop);
    }
  } catch (e) {}
  return false;
}
 ...
 * SIGTERM to the running process. When the running talnetrouter cli.js was itself
...
<dict>
    ...
    <string>/tmp/talnetrouter.log</string>
    <key>StandardErrorPath</key>
    <string>/tmp/talnetrouter.error.log</string>
</dict>
</plist>`;
...
function enableWindows(cliPath) {
  ...
  // `talnetrouter.cmd` lookup that depended on the npm prefix path.
...
function disableWindows() {
  for (const name of [APP_NAME, LEGACY_APP_NAME]) {
    const vbsPath = path.join(process.env.APPDATA || "", "Microsoft", "Windows", "Start Menu", "Programs", "Startup", `${name}.vbs`);
...
Type=Application
Name=TalNet Router
Comment=TalNet Router API Proxy
...
function disableLinux() {
  for (const name of [APP_NAME, LEGACY_APP_NAME]) {
    const desktopPath = path.join(os.homedir(), ".config", "autostart", `${name}.desktop`);
    if (fs.existsSync(desktopPath)) {
      try { fs.unlinkSync(desktopPath); } catch {}
    }
  }
  return true;
}
