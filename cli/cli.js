...
// Subcommands (`talnetrouter xai video …`, legacy `9router xai video …`) run against an already-running gateway
...
if (process.argv[1] && path.basename(process.argv[1]).replace(/\.js$/i, "") === "9router") {
  console.warn("⚠️  `9router` is deprecated — use `talnetrouter` instead (alias still works).");
}
// Self-heal SQLite runtime deps (sql.js + better-sqlite3) into ~/.talnetrouter/runtime
...
function getAppDataDir() {
  ...
  const primary = process.platform === "win32"
    ? path.join(process.env.APPDATA || "", "talnetrouter")
    : path.join(os.homedir(), ".talnetrouter");
  try {
    if (!fs.existsSync(primary) && fs.existsSync(getLegacyAppDataDir())) {
      ...
      copyRecursive(getLegacyAppDataDir(), primary);
    }
  } catch {}
  return primary;
}
      ...
      if (platform === "win32") {
        ...
        try {
          ...
          lines.forEach(line => {
            ...
            const isAppProcess =
              (cmd.includes("node") && (cmd.includes("talnetrouter") || cmd.includes("9router")) && (cmd.includes("cli.js") || cmd.includes("\\talnetrouter") || cmd.includes("/talnetrouter") || cmd.includes("\\9router") || cmd.includes("/9router")))
        ...
        try {
          ...
          lines.forEach(line => {
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && (cmd.includes("talnetrouter") || cmd.includes("9router")) && (cmd.includes("cli.js") || cmd.includes("/talnetrouter") || cmd.includes("/9router")))
              || cmd.includes("next-server");
            if (isAppProcess) {
              const parts = line.trim().split(/\s+/);
              const pid = parts[1];
...
function startServer(updatePromise) {
      ...
      while (true) {
        ...
        if (choice === "update") {
          ...
          if (process.platform === "darwin") {
            ...
            console.log(`🔔 TalNet Router is running in tray (PID: ${process.pid})`);
            console.log(`   Server: http://${displayHost}:${port}`);
          ...
          console.log(`🔔 TalNet Router is now running in background (PID: ${bgProcess.pid})`);
          console.log(`   Server: http://${displayHost}:${port}`);
          console.log(`\n💡 You can close this terminal. Right-click tray icon to quit.\n`);
  ...
  function tryRestart(code) {
    ...
    if (restartCount >= MAX_RESTARTS) {
      ...
      try {
        const dbPath = (() => { try { return path.join(getAppDataDir(), "db.json"); } catch { return path.join(os.homedir(), process.platform === "win32" ? path.join("AppData", "Roaming", "talnetrouter", "db.json") : path.join(".talnetrouter", "db.json")); } })();
