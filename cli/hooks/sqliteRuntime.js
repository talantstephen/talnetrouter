...
function getDataDir() {
  ...
  const primary = process.platform === "win32"
    ? path.join(process.env.APPDATA || os.homedir(), "talnetrouter")
    : path.join(os.homedir(), ".talnetrouter");
  try {
    if (!fs.existsSync(primary) && fs.existsSync(getLegacyDataDir())) {
      ...
      copyRecursive(getLegacyDataDir(), primary);
    }
  } catch {}
  return primary;
}
...
function ensureRuntimeDir() {
  ...
  if (!fs.existsSync(pkgPath)) {
    fs.writeFileSync(pkgPath, JSON.stringify({
      ...
      version: "1.0.0",
      private: true,
      description: "User-writable runtime deps for talnetrouter (better-sqlite3 native binary)",
    }, null, 2));
  }
  return dir;
}
