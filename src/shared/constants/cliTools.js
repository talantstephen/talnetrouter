...
# cs45 -> cc/claude-sonnet-4-5-20250929`,
  ...
  jcode: {
    ...
    notes: [
      ...
      {
        type: "info",
        text: "Configure TalNet Router as an OpenAI-compatible provider to route all jcode requests through TalNet Router's optimization layer."
      },
      {
        type: "warning",
  ...
  "grok-build": {
    ...
    notes: [
      {
        type: "info",
        text: "Grok Build uses ~/.grok/config.toml. TalNet Router writes a [model.9router] custom model and sets it as the default.",
      },
      {
        type: "info",
...
devin --version`,
  ...
  opendesign: {
    ...
    configType: "guide",
    notes: [
      { type: "info",         text: "OpenDesign ships as a plugin/skills pack installed into Claude Code, Cursor, OpenAI Codex, Gemini CLI, or OpenCode. It inherits the host agent's model config, so once your host points at TalNet Router, /opendesign design sessions route through TalNet Router automatically — no extra env vars needed." },
    ...
    guideSteps: [
      { step: 1, title: "Install the plugin", desc: "Pick your host below and run the matching install command from the matrix." },
       { step: 2, title: "No config needed", desc: "OpenDesign runs inside your host agent and uses its model config. If the host already routes through TalNet Router, /opendesign traffic does too." },
