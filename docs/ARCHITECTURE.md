# TalNet Router Architecture

_Last updated: 2026-02-06_

## Executive Summary

TalNet Router is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[TalNet Router Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.talnetrouter/db.json` when `DATA_DIR` is unset, legacy `~/.9router` migrated)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.talnetrouter/usage.json`, `~/.talnetrouter/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.talnetrouter/db.json`)
- usage stats: `~/.talnetrouter/usage.json`
- request log lines: `~/.talnetrouter/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[TalNet Router Runtime]
        Next[Next.js Server\nPORT=20128]
...
1. `usageDb` currently stores under `~/.talnetrouter` (legacy `~/.9router` migrated) and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/talnetrouter && npm run build`
- Build Docker image: `cd /root/dev/talnetrouter && docker build -t talnetrouter .`
